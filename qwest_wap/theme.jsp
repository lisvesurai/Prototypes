<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE html PUBLIC "-//WAPFORUM//DTD XHTML Mobile 1.0//EN"
"http://www.wapforum.org/DTD/xhtml-mobile10.dtd">
<html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
<head>
	<title>Qwest</title>
	<link href="include/fragments/main.css" rel="stylesheet" type="text/css" />
</head>
<body>
<div id="logo"><img src="images/logo.gif" alt="Qwest" /></div>
	<div class="h1">THEMES</div>

	<div class="banner">
		<a href="theme_details.jsp"><img src="images/theme_banner.gif" alt="Banner" class="banner"></a>
	</div>

	<dl>
		<dt class="mnr">1 <a href="theme_top.jsp" accesskey="1">Top Themes</a></dt>
		<dt class="mnr">2 <a href="#" accesskey="2">New Releases</a></dt>
		<dt class="mnr">3 <a href="#" accesskey="3">Bling Pix</a></dt>
		<dt class="mnr">4 <a href="#" accesskey="4">Rep Your Hood</a></dt>
		<dt class="mnr">5 <a href="theme_by_genre.jsp" accesskey="5">Browse by Genre</a></dt>
		<dt class="mnr">6 <a href="theme_by_title.jsp" accesskey="6">Browse By Title</a></dt>
		<dt class="mnr">7 <a href="theme_vault.jsp" accesskey="7">Vault - Themes</a></dt>
	</dl>
	
	<jsp:include page="inc_search.jsp" flush="true"/>

	<dl class="footer">
		<dt>&lt; <a href="downloads.jsp"> qZone</a></dt>
		<dt>&lt; <a href="waphome.jsp"> Qwest Home</a></dt>
	</dl>

</body>
</html>