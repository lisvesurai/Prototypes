...
<jsp:include page="inc_header.jsp" flush="true"/>
	...
	<div class="content">
			...
			<table>
				<tr>
					...
					<td class="item" valign="top">
                        ...
                        <p class="price"><b>$4.99</b></p>
                        <a href="redownload.jsp?w=<%=screenWidth%>&f=<%=fontSize%>&s=<%=siteStr%>&tab=<%=tabStr%>"><img src="UI/img/<%=screenWidth%>/btn_download.gif" alt="Download" /></a>
					</td>
				</tr>
			</table>
			<%=msgStr%>
		...
		<div class="contentpad">
			<p class="price"><b>$4.99</b> <a href="redownload.jsp?w=<%=screenWidth%>&f=<%=fontSize%>&s=<%=siteStr%>&tab=<%=tabStr%>"><img src="UI/img/<%=screenWidth%>/btn_download.gif" alt="Download" /></a>
		</div>
		<div class="link">
			<dl class="mnr">
				<% if (msg.equals("6")) { %>
				<dt class="mnr">&nbsp;&gt; <a href="item_own.jsp?w=<%=screenWidth%>&f=<%=fontSize%>&s=<%=siteStr%>&tab=<%=tabStr%>&msg=7" class="mnr">Remove from wishlist</a></dt>
				<% } else { %>
				<dt class="mnr">&nbsp;&gt; <a href="item_own.jsp?w=<%=screenWidth%>&f=<%=fontSize%>&s=<%=siteStr%>&tab=<%=tabStr%>&msg=6" class="mnr">Add to wishlist</a></dt>
				<% } %>
				<dt class="mnr">&nbsp;&gt; <a href="search_results.jsp?w=<%=screenWidth%>&f=<%=fontSize%>&s=<%=siteStr%>&tab=<%=tabStr%>" class="mnr">More from Gameloft</a></dt>
			</dl>
		</div>
