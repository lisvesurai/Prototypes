<%@ include file = "../template/virgin/tools.jsp" %>

		<h2 class="hdl" id="hdl_downloads">Downloads</h2>

		<jsp:include page="../template/virgin/sidebar.jsp" flush="true">
			<jsp:param name="hl" value="0"/>
		</jsp:include>
		
		<div class="col main1 activate">
			<div class="main45">
						<div id="ringtone_header">
							<h2 class="hdl" id="hdl_search_results">Search Results</h2>
							<h4 id="no_annual_contract">No Annual Contract</h4>
						</div>

				<jsp:include page="../template/virgin/device.jsp" flush="true"/>
			</div>
			
			<h2 class="breadcrumb">Your search for <b>"Lorem ipsum"</b> returned 345 results.</h2>

					<div class="border">
						<table cellspacing="0" class="styled3">
							<tr>
								<td colspan="6"><h4>Ringback Tones</h4></td>
							</tr>
							<tr>
								<td colspan="6" class="pagination"><ul>
										<li>Ringback Tones 1-20 of 14496</li>
										<li><a href="#" class="selected">1</a></li>
										<li><a href="#">2</a></li>
										<li><a href="#">3</a></li>
										<li><a href="#">4</a></li>
										<li><a href="#">5</a></li>
										<li><a href="#">6</a></li>
										<li><a href="#">7</a></li>
										<li><a href="#">8</a></li>
										<li><a href="#">9</a></li>
										<li><a href="#">10</a></li>
										<li><a href="#">Next 10</a></li>
									</ul></td>
							</tr>
							<tr>
								<th class="borderbottom">Artist</th>
								<th class="borderbottom">Title</th>
								<th class="borderbottom">Type</th>
								<th class="borderbottom">&nbsp;</th>
								<th class="borderbottom align_right">Price</th>
								<th class="borderbottom">&nbsp;</th>
							</tr>
														<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Bye Bye</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kiss Me Thru The Phone</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kanye West</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Heartless</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Beyonce</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Diva</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Keyshia Cole</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">You Complete Me</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">NeYo</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Mad</a></th>
								<td>Polyphonic Tones</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Bye Bye</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kiss Me Thru The Phone</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kanye West</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Heartless</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Beyonce</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Diva</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Keyshia Cole</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">You Complete Me</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">NeYo</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Mad</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Bye Bye</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kiss Me Thru The Phone</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kanye West</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Heartless</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Beyonce</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Diva</a></th>
								<td>Polyphonic Tones</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Keyshia Cole</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">You Complete Me</a></th>
								<td>Polyphonic Tones</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">NeYo</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Mad</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Bye Bye</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kiss Me Thru The Phone</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Kanye West</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Heartless</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Beyonce</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Diva</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Keyshia Cole</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">You Complete Me</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td><a href="ct_search_results.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">NeYo</a></td>
								<th><a href="ct_detail.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Mad</a></th>
								<td>Real Music</td>
								<td><a href="mms://stream2.modtones.com:554/wma/508855.wma"><img src="_img/listen.gif" onmouseover="this.src='_img/listen_over.gif';" onmouseout="this.src='_img/listen.gif';" alt="listen" /></a></td>
								<td class="align_right">$2.50</td>
								<td class="align_right"><span class="buy_now"><a href="ct_buyConfirm.jsp?u=<%=loginStatus%>&p=<%=phone%>&pn=<%=phoneName%>&credit=<%=haveCredit%>&c=ct">Buy Now</a></span></td>
							</tr>
							<tr>
								<td colspan="6" class="pagination"><ul>
										<li>Ringback Tones 1-20 of 14496</li>
										<li><a href="#" class="selected">1</a></li>
										<li><a href="#">2</a></li>
										<li><a href="#">3</a></li>
										<li><a href="#">4</a></li>
										<li><a href="#">5</a></li>
										<li><a href="#">6</a></li>
										<li><a href="#">7</a></li>
										<li><a href="#">8</a></li>
										<li><a href="#">9</a></li>
										<li><a href="#">10</a></li>
										<li><a href="#">Next 10</a></li>
									</ul></td>
							</tr>
						</table>
					</div>
</div>